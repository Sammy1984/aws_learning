...
do
	...
	key_rsa=$(aws ec2 describe-key-pairs --region $reg --filters "Name=key-name,Values=student-rsa" --query "KeyPairs[].KeyName" --output text) > /dev/null
	echo "$key_rsa - OK"	
	key_ed25519=$(aws ec2 describe-key-pairs --region $reg --filters "Name=key-name,Values=student-ed25519" --query "KeyPairs[].KeyName" --output text) >/dev/null
	echo "$key_ed25519 - OK"
